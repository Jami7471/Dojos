...
namespace AdventOfCode.Year2022
{
    public sealed class Task10 : BaseTask
    {
        ...
        Task: Find the signal strength during the 20th, 60th, 100th, 140th, 180th, and 220th cycles. What is the sum of these six signal strengths?
        Solution: 14220
        ...
        Task: Render the image given by your program. What eight capital letters appear on your CRT?
        Solution: ZRARLFZU
        ...
        */
        ...
        {
            List<string> input = ReadInputLines(@"Year2022\Input\Input10.txt");
            ...
            {
                DoInstruction(input[i], ref registerX, ref cycle, ref signalStrength);
            }
        ...
        public override string Part2()
        {
            ...
            List<int[]> ctr = GetCtrTable();
            SetCtrPixel(input, ref registerX, ref cycle, ref currentPixel, ref currentRow, ctr);

            StringBuilder ctrDrawString = GetCtrDrawString(ctr);
            return ctrDrawString.ToString();
        }

        private StringBuilder GetCtrDrawString(List<int[]> ctr)
        {
            StringBuilder ctrDrawString = new();
            ctrDrawString.AppendLine("");
            ...
            return ctrDrawString;
        }

        private void SetCtrPixel(List<string> input, ref int registerX, ref int cycle, ref int currentPixel, ref int currentRow, List<int[]> ctr)
        {
            for (int i = 0; i < input.Count; i++)
            {
                DoInstruction(input[i], ref registerX, ref cycle, ref currentPixel, ref currentRow, ctr);
            }
        }

        private List<int[]> GetCtrTable()
        {
            List<int[]> ctr = new();
        ...
        private void DoInstruction(string instruction, ref int registerX, ref int cycle, ref int currentPixel, ref int currentRow, List<int[]> ctr)
        {
            DetermineCurrentCtrPosition(ref currentPixel, ref currentRow);
            cycle++;
            SetPixel(registerX, currentPixel, currentRow, ctr);
            ...
            if (instruction != "noop")
            {
                ...
                cycle++;
                SetPixel(registerX, currentPixel, currentRow, ctr);
                int lastIndexOfSpace = instruction.LastIndexOf(" ");
                registerX += int.Parse(instruction.Substring(lastIndexOfSpace));
            }
        }

        private void SetPixel(int registerX, int currentPixel, int currentRow, List<int[]> ctr)
        {
            if (currentPixel >= registerX - 1 && registerX + 1 >= currentPixel)
            {
                ctr[currentRow][currentPixel] = 1;
            }
            else
            {
                ctr[currentRow][currentPixel] = 0;
            }
        }

        private void DetermineCurrentCtrPosition(ref int currentPixel, ref int currentRow)
        {
            currentPixel++;
        ...
        private void DoInstruction(string instruction, ref int registerX, ref int cycle, ref int signalStrength)
        {
            cycle++;
            signalStrength += GetSignalStrength(cycle, registerX, 20, 11, true);
        ...
        private int GetSignalStrength(int cycle, int registerX, int factor, int factorRange, bool onlyOdd = false)
        {
