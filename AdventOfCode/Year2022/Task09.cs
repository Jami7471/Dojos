...
namespace AdventOfCode.Year2022
{
    public sealed class Task09 : BaseTask
    {
        ...
        Task: Simulate your complete hypothetical series of motions. How many positions does the tail of the rope visit at least once?
        Solution: 6339
        ...
        */
        ...
        {
            List<string> input = ReadInputLines(@"Year2022\Input\Input09.txt");

            List<Point> points = CreatePoints(2);
            List<List<int>> map = InitializeMap();

            foreach (string move in input)
            {
                string[] instruction = move.Split(' ');
                DoSteps(points, map, instruction[0], int.Parse(instruction[1]));
            }

            return CountOfVisitNodes(map).ToString();
        }
        ...
        public override string Part2()
        {
            ...
            List<Point> points = CreatePoints(10);
            List<List<int>> map = InitializeMap();
        ...
        private static List<List<int>> InitializeMap()
        {
        ...
        private int CountOfVisitNodes(List<List<int>> map)
        {
            int visitNode = 0;

            for (int i = 0; i < map.Count; i++)
            {
                visitNode += map[i].Count(node => node > 0);
            }

            return visitNode;
        }

        private void DoSteps(List<Point> points, List<List<int>> map, string direction, int steps)
        {
            for (int step = 0; step < steps; step++)
            {
                ExecuteDirection(points, map, direction);
                CheckPointsInRange(points, map, direction);
            }
        }

        private void CheckPointsInRange(List<Point> points, List<List<int>> map, string direction)
        {
            for (int i = 1; i < points.Count; i++)
            {
                ...
                if (IsInRange(firstPoint, secondPoint) == false)
                {
                    ...
                    points[i] = secondPoint;

                    if (i == points.Count - 1)
                    {
                        map[secondPoint.X][secondPoint.Y]++;
                    }
                }
                else
                {
        ...
        private void ExecuteDirection(List<Point> points, List<List<int>> map, string direction)
        {
            switch (direction)
            {
                case "R":
                    MoveHeadRight(points, map);
                    break;
                case "U":
                    MoveHeadUp(points, map);
                    break;
                case "L":
                    MoveHeadLeft(points, map);
                    break;
                case "D":
                    MoveHeadDown(points, map);
                    break;
                default:
        ...
        private void MoveHeadDown(List<Point> points, List<List<int>> map)
        {
            if (points.Count == 0)
            {
                return;
            ...
            if (map[point.X].Count <= point.Y + 1)
            {
                AddNewRow(map);
            }
        ...
        private void MoveHeadLeft(List<Point> points, List<List<int>> map)
        {
            if (points.Count == 0)
            {
                return;
        ...
        private void MoveHeadUp(List<Point> points, List<List<int>> map)
        {
            if (points.Count == 0)
            {
                return;
        ...
        private void MoveHeadRight(List<Point> points, List<List<int>> map)
        {
            if (points.Count == 0)
            {
                return;
        ...
        private void AddNewColumn(List<List<int>> map)
        {
            map.Add(new List<int>());
            map[map.Count - 1].AddRange(new int[map[0].Count]);
        }

        private void AddNewRow(List<List<int>> map)
        {
            for (int i = 0; i < map.Count; i++)
            {
                map[i].Add(0);
        ...
        private void InsertNewColumn(List<Point> points, List<List<int>> map)
        {
            map.Insert(0, new List<int>(map[0].Count));
        ...
        private void InsertNewRow(List<Point> points, List<List<int>> map)
        {
            for (int i = 0; i < map.Count; i++)
            {
                map[i].Insert(0, 0);
        ...
        private bool IsInRange(Point firstPoint, Point secondPosition)
        {
            if (firstPoint.X == secondPosition.X && firstPoint.Y == secondPosition.Y)
            {
                return true;
            }
            else if (firstPoint.X != secondPosition.X && firstPoint.Y == secondPosition.Y)
            {
                int diff = firstPoint.X - secondPosition.X;
                diff = (diff < 0) ? diff * -1 : diff;
                return (diff == 1);
            }
            else if (firstPoint.X == secondPosition.X && firstPoint.Y != secondPosition.Y)
            {
                int diff = firstPoint.Y - secondPosition.Y;
                diff = (diff < 0) ? diff * -1 : diff;
                return (diff == 1);
            }
            else
            {
                int diffX = firstPoint.X - secondPosition.X;
                diffX = (diffX < 0) ? diffX * -1 : diffX;

                int diffY = firstPoint.Y - secondPosition.Y;
