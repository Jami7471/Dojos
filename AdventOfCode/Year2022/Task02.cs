...
namespace AdventOfCode.Year2022
{
    public sealed class Task02 : BaseTask
    {
        ...
        */
        ...
        {
            List<string> input = ReadInputLines(@"Year2022\Input\Input02.txt");

            int myScore = 0;

            foreach(string round in input)
            {
                string[] choose = round.Split(' ');
                PlayRound(ref myScore, choose);
            }
        ...
        public override string Part2()
        {
            ...
            {
                string[] choose = round.Split(' ');
                PlayRound2(ref myScore, choose);
            }

            return myScore.ToString();
        }

        private void PlayRound(ref int myScore, string[] choose)
        {
            string opponentChoose = choose[0];
            string myChoose = ConvertMyChooseToOpponentChooseValues(choose[1]);

            int roundResultForMe = GetRoundResultForMe(opponentChoose, myChoose);

            myScore += ConvertChooseToValue(myChoose) + GetValueByRoundResult(roundResultForMe);            
        }

        private void PlayRound2(ref int myScore, string[] choose)
        {
            string opponentChoose = choose[0];
            string myChoose = DetermineMyChoose(opponentChoose, choose[1]);
        ...
        private string DetermineMyChoose(string opponentChoose, string roundStatement)
        {
            switch (roundStatement)
            {
                case "Y":
                ...
                case "X":
                    switch(opponentChoose)
                    {
                        ...
                        case "C":
                            return "B";
                        default:
                            throw new ArgumentException($"no mapping found: '{opponentChoose}'");
                    }
                case "Z":
                    switch (opponentChoose)
                    {
                ...
                default:
                    throw new ArgumentException($"no mapping found: '{roundStatement}'");
            }
        }

        private string ConvertMyChooseToOpponentChooseValues(string myChoose)
        {
            switch(myChoose)
            {
                case "Y":
                ...
                case "Z":
                    return "C";
                default:
                    throw new ArgumentException($"no mapping found: '{myChoose}'");
            }
        }

        private int ConvertChooseToValue(string choose)
        {
            switch (choose)
            {
                case "A":
                ...
                case "C":
                    return 3;
                default:
                    throw new ArgumentException($"no mapping found: '{choose}'");
            }
        }

        private int GetValueByRoundResult(int roundResult)
        {
            switch (roundResult)
            {
                case -1:
                ...
                case 1:
                    return 6;
                default:
                    throw new ArgumentException($"no mapping found: '{roundResult}'");
            }
        }

        private int GetRoundResultForMe(string opponentChoose, string myChoose)
        {
            if(opponentChoose == myChoose)
            {
                return 0;
            }
            else
