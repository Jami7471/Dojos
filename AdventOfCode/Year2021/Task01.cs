...
namespace AdventOfCode.Year2021
{
    internal class Task01
    {
        ...
        */
        ...
        public string Do()
        {
            return $"{Environment.NewLine}\tResult1: {Part1(ReadInputTxt())}{Environment.NewLine}\tResult2: {Part2(ReadInputTxt())}";
        }

#nullable disable warnings
        private List<string> ReadInputTxt()
        {
            List<string> inputTxt = new();

            string txtFile = Path.Combine(AppContext.BaseDirectory, @"Year2021\Input\Input01.txt");
...
#nullable enable warnings
        ...
        private string Part1(List<string> myPuzzleInput)
        {
            int countOfIncreased = 0;
            int previousMeasurement = -1;

            foreach (string measurement in myPuzzleInput)
            {
                if (int.TryParse(measurement, out int currentMeasurement))
                {
                    if (previousMeasurement > 0)
                    {
                        if (IsLargerThanThePreviousMeasurement(previousMeasurement, currentMeasurement))
                        {
                            countOfIncreased++;
                        }
                    }
                ...
                {
                    throw new InvalidCastException($"'{measurement}' is not numeric");
                }
            }

            return countOfIncreased.ToString();
        }

        private string Part2(List<string> myPuzzleInput)
        {
            int countOfIncreased = 0;
            int previousMeasurement = -1;

            for (int i = 0; i < myPuzzleInput.Count - 2; i++)
            {
                int currentMeasurement = GetIntValue(myPuzzleInput[i]) + GetIntValue(myPuzzleInput[i + 1]) + GetIntValue(myPuzzleInput[i + 2]);
        ...
        private bool IsLargerThanThePreviousMeasurement(int previousMeasurement, int currentMeasurement)
        {
